...
This package is a modern alternative to the R standard function, `install.packages()`. Is goal is make package installation faster and more reliable. In particular, it performs all HTTP operations in parallel, so metadata resolution and package downloads are fast. In addition, he has a dependency solver, so it finds version conflicts before performing the installation. Futhermore, he supports CRAN, 'Bioconductor' and 'GitHub' packages as well.
